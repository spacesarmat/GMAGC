# ruff: noqa: E501 - данные путей SVG длинные по природе
"""Иконки боковой панели: пути Material Icons (24×24) как встроенные SVG, окрашиваются нужным цветом."""
...
def icon(name: str, color: str, size: int = 18) -> QIcon:
    """Иконка `name` цвета `color`; неизвестное имя даёт пустую иконку."""
    path = _PATHS.get(name)
    if path is None:
        return QIcon()
    svg = f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="{color}" d="{path}"/></svg>'
    renderer = QSvgRenderer(QByteArray(svg.encode("utf-8")))
    pixmap = QPixmap(size * 2, size * 2)  # ×2 — чёткость на экранах с масштабом
